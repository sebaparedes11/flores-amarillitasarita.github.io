...
function DibujarPetalo(x, y, RadioX, scala, Rotacion, color, pasos) {
    ...
    // Añade un gradiente de color para dar un efecto de sombreado y amarillo
    const gradient = ctx.createLinearGradient(0, 0, 0, RadioX);
    gradient.addColorStop(0, '#F8E231'); // Color amarillo claro y brillante
    gradient.addColorStop(0.3, '#F2C464'); // Color amarillo medio claro
    gradient.addColorStop(0.7, '#F5DEB3'); // Color amarillo medio
    gradient.addColorStop(1, '#F8E231'); // Color amarillo claro y brillante
    ...
    // Añade un borde suave para dar un efecto de textura
    ctx.strokeStyle = '#F5DEB3'; // Color amarillo claro
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    if (NuevaY < y + AltoTrazo) {
      ...
      function DibujarPetalosTallo() {
        if (CuantosPasos <= Pasos) {
            ...
            DibujarPetalo(500, PetaloY, 15, 2, 300, 'green', CuantosPasos, Angulo);
            DibujarPetalo(470, PetaloY2, 15, 2, 300, 'green', CuantosPasos, Angulo);
            CuantosPasos++;
            Angulo += 10; // Añade un ángulo de rotación para cada pétalo
